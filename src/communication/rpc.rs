...
mod tests {
    ...
    test_rpc!(
        test_rpc_send_tree,
        RPC::SendTreeFileStructure {
            tree: vec!["to/path/a".to_string(), "to/path/b".to_string()],
        }
    );
    ...
    fn test_rpc_security() {
        ...
        println!("{:?}", encoded);
        let (decode, len): (RPC, usize) = bincode::decode_from_slice(&encoded[..], config)
            .unwrap_or_else(|e| {
                println!("{:?}", e);
                (
                    RPC::ReadBuffer {
